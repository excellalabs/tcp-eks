...
gcloud + kubectl
kubectl is the de facto command line tool for interacting with your Kubernetes cluster. If you hare already using the gcloud command line tool then its likely you have kubectl installed, but if not you can download it here. It can be configured to control multiples clusters, each within their own "context". So to begin, we will use gcloud to authenticate with Kubernetes and create a context for kubectl.
...
So create a Google Cloud Storage bucket (change your-project-id accordingly):
